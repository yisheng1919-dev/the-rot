...
export class Room {
  ...
  _startFreeRoam() {
    ...
    // Failsafe: if nobody ever walks to Main Hall and calls a meeting, the
  ...
  // Teleports every living player to Main Hall (seated "around the table"),
  ...
  _gatherForMeeting(calledByName) {
    ...
    const seats = this._generateSpawnPoints(living.length, 0, -1, 2.6); // Main Hall center
    living.forEach((p, i) => {
      p.x = seats[i].x;
      p.z = seats[i].z;
    });
  ...
  handleCallMeeting(playerId) {
    ...
    if (this.phase === PHASES.FREE_ROAM) {
      if (!pointInZone(player.x, player.z, ROOMS.MAIN_HALL)) {
        return { ok: false, reason: "You must be in the Main Hall to call a meeting." };
      }
    ...
    if (this.phase === PHASES.DISCUSSION) {
      // A second call during an active discussion cuts the remaining time
      // down to a short countdown instead of waiting out the full timer.
