...
export class GameScene2D {
  ...
  // Chamfered-corner octagon — used for ROOMS specifically, to read more
  // like a built station chamber (see reference room-map art) than a
  // simple rounded rectangle. Corridors stay as plain rounded rects since
  // in that reference they're simple straight connecting halls, not rooms.
  ...
  // Dispatcher used throughout _drawZone so every fill/stroke/clip call
  // automatically gets the right silhouette for what it's drawing.
  ...
  // Simple geometric furniture silhouettes per room — not fine art, but
  // enough visual clutter that rooms read as occupied spaces rather than
  // empty color blocks. Drawn in local room-floor space (0..w, 0..h).
  _drawRoomProps(zone, sx, sy, w, h) {
    const ctx = this.ctx;
    ctx.save();
    ctx.translate(sx, sy);
    ctx.globalAlpha = 0.5;
    ...
    switch (zone.id) {
      case "POWER_ROOM": {
        // A generator core with radiating spokes, centered.
      ...
      case "MAP_ROOM": {
        // A console table along the back wall.
        rr(w * 0.2, h * 0.15, w * 0.6, h * 0.22, 6);
        ctx.fill();
        ctx.stroke();
        break;
      }
      case "LOUNGE": {
        // A round table with a few chair-dots around it.
        const cx = w / 2, cy = h / 2, r = Math.min(w, h) * 0.22;
        ctx.beginPath();
        ctx.arc(cx, cy, r, 0, Math.PI * 2);
        ctx.fill();
        ...
        for (let i = 0; i < 4; i++) {
          ...
          ctx.beginPath();
          ctx.arc(cx + Math.cos(a) * (r + 16), cy + Math.sin(a) * (r + 16), 6, 0, Math.PI * 2);
          ctx.fill();
        }
        break;
      }
      case "STORAGE": {
        // Stacked crates.
        const crates = [
          [w * 0.15, h * 0.2, w * 0.22, h * 0.22],
          [w * 0.4, h * 0.15, w * 0.2, h * 0.2],
          [w * 0.2, h * 0.55, w * 0.25, h * 0.25],
          [w * 0.55, h * 0.5, w * 0.28, h * 0.28],
        ];
        for (const [x, y, ww, hh] of crates) {
          rr(x, y, ww, hh, 4);
          ctx.fill();
          ctx.stroke();
        }
      ...
      case "REST_AREA": {
        // Two beds side by side (rounded rect body + pillow).
        for (const offsetX of [w * 0.12, w * 0.56]) {
          rr(offsetX, h * 0.55, w * 0.32, h * 0.32, 8);
          ctx.fill();
          ctx.stroke();
          rr(offsetX + 4, h * 0.58, w * 0.1, h * 0.1, 4);
          ctx.fill();
        }
        break;
      }
      case "FIRST_AID": {
        // A medical cross plus a cabinet.
        const cx = w * 0.7, cy = h * 0.35, s = Math.min(w, h) * 0.1;
        ctx.fillStyle = "rgba(255,120,140,0.35)";
        rr(cx - s * 0.35, cy - s, s * 0.7, s * 2, 2);
        ctx.fill();
        rr(cx - s, cy - s * 0.35, s * 2, s * 0.7, 2);
        ctx.fill();
        ctx.fillStyle = "rgba(0,0,0,0.22)";
        rr(w * 0.15, h * 0.55, w * 0.35, h * 0.3, 6);
        ctx.fill();
        ctx.stroke();
        break;
      }
      case "CONTROL_ROOM": {
        // Bank of monitor consoles along the wall.
        for (let i = 0; i < 3; i++) {
          rr(w * 0.15 + i * w * 0.24, h * 0.15, w * 0.18, h * 0.16, 3);
          ctx.fill();
      ...
      case "MAIN_HALL": {
        // A couple of small potted plants near the entrances.
        for (const [x, y] of [[w * 0.15, h * 0.85], [w * 0.85, h * 0.85]]) {
          ctx.beginPath();
          ctx.arc(x, y, 7, 0, Math.PI * 2);
          ctx.fillStyle = "rgba(90,180,120,0.3)";
          ctx.fill();
        }
        break;
      }
  ...
  _drawZone(zone, isRoom) {
    ...
    const ctx = this.ctx;
    const radius = isRoom ? 22 : 4; // chamfer size for rooms, corner radius for corridors
    ...
    // 3b) Subtle songket/batik lattice texture, a hibiscus medallion in
    // Main Hall, simple furniture silhouettes, and (3c) The Rot's cracks —
    // all clipped to the floor shape.
    if (isRoom) {
      ctx.save();
      this._drawShape(sx, sy, w, h, isRoom, radius);
      ctx.clip();
      ...
      this._drawRoomProps(zone, sx, sy, w, h);
