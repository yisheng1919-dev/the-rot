...
export class GameScene2D {
  constructor(container) {
    ...
    this.scale = 24; // slightly zoomed out from before — the map is bigger now
  ...
  _charImage(color) {
    const key = `char:${color || "beige"}`;
    return this._getImage(key, spriteUrlFor(color || "beige"));
  }
  ...
  _drawPlayer(p) {
    ...
    const ctx = this.ctx;
    const img = this._charImage(p.color);
